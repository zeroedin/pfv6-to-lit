...
- The scope is `@pfv6` NOT `@patternfly` for Lit element imports
...
4. **JSDoc comments** (minimal during api-writer phase)
   - Add basic component description
   - Document `@slot` entries for slots visible in render()
   - Document `@fires` entries for Event classes
   - **Note**: Full `@cssprop` documentation is generated by `jsdoc-writer` after CSS is created
