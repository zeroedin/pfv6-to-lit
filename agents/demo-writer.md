...
**Note**: Don't use `component="li"` when wrapped in `<li>` - that creates redundant list item semantics.
...
**Why React demos may differ**:
- React: `<Divider component="li" />` compiles to `<li>` (valid HTML)
- Lit: `<pfv6-divider component="li">` stays as custom element (invalid as direct `<ul>` child)

**Solution**: Wrap the custom element in the appropriate parent element.

## Step 6: Detect and Translate Layout Components

Before converting React component demos, identify any layout components and delegate their translation to the `layout-translator` agent.
...
- Use `component="li"` when already wrapped in `<li>`
