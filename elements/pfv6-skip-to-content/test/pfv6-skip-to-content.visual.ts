...
const litDemos = discoverDemos('skip-to-content');
  ...
  litDemos.forEach(demoName => {
    test(`Parity: ${demoName} (Lit vs React)`, async ({ page, browser }) => {
      ...
      try {
        ...
        await reactPage.goto(`/elements/pfv6-skip-to-content/react/test/${demoName}`);
        await waitForFullLoad(reactPage);

        await page.goto(`/elements/pfv6-skip-to-content/test/${demoName}`);
