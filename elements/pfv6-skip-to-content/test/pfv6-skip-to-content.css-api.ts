...
/**
 * CSS API Tests for pfv6-skip-to-content
 *
...
const cssApiTests = [
  ...
  },
  {
    ...
    defaultValue: 'var(--pf-t--global--z-index--2xl, 600)',
    resolvedValue: '600',
    type: 'number',
    testValue: '999',
    demo: 'basic',
  ...
  cssApiTests.forEach(({ name, defaultValue, resolvedValue, type, testValue, demo }) => {
    test(`CSS API: ${name}`, async ({ page, browser }) => {
      ...
      try {
        ...
        await reactPage.goto(`/elements/pfv6-skip-to-content/react/test/${demo}`);
        await applyCssOverride(reactPage, '.pf-v6-c-skip-to-content', name, testValue);
        await waitForFullLoad(reactPage);

        // Load Lit demo with CSS override
        await page.goto(`/elements/pfv6-skip-to-content/test/${demo}`);
        await applyCssOverride(page, 'pfv6-skip-to-content', name, testValue);
