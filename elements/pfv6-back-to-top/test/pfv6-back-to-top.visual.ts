...
const litDemos = discoverDemos('back-to-top');
  ...
  litDemos.forEach(demoName => {
    test(`Parity: ${demoName} (Lit vs React)`, async ({ page, browser }) => {
      ...
      try {
        ...
        await reactPage.goto(`/elements/pfv6-back-to-top/react/test/${demoName}`);
        await waitForFullLoad(reactPage);

        await page.goto(`/elements/pfv6-back-to-top/test/${demoName}`);
