/**
 * Visual parity tests for pfv6-spinner component
 *
...
const litDemos = discoverDemos('spinner');
  ...
  litDemos.forEach(demoName => {
    test(`Parity: ${demoName} (Lit vs React)`, async ({ page, browser }) => {
      ...
      try {
        ...
        await reactPage.goto(`/elements/pfv6-spinner/react/test/${demoName}`);
        await waitForFullLoad(reactPage);

        await page.goto(`/elements/pfv6-spinner/test/${demoName}`);
        await waitForFullLoad(page);

        // Take FRESH screenshots (no baseline files)
        // Note: Spinner has animations, but they are disabled via screenshot options
