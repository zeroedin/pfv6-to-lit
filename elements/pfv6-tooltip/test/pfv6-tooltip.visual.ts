...
// Helper to trigger tooltip visibility and wait for it to be visible
async function showTooltip(page: Page): Promise<void> {
  // Hover over the trigger element to show the tooltip
  const trigger = page.locator('pfv6-tooltip').first();
  await trigger.hover();

  // Wait for tooltip to be visible (account for entry delay + animation)
  await page.waitForSelector('#tooltip', { state: 'visible', timeout: 5000 });
...
const litDemos = discoverDemos('tooltip');
  ...
  litDemos.forEach(demoName => {
    test(`Parity: ${demoName} (Lit vs React)`, async ({ page, browser }) => {
      ...
      try {
        ...
        await reactPage.goto(`/elements/pfv6-tooltip/react/test/${demoName}`);
        await waitForFullLoad(reactPage);

        await page.goto(`/elements/pfv6-tooltip/test/${demoName}`);
        await waitForFullLoad(page);

        // Show tooltip on both pages (tooltips are hidden by default)
        await showTooltip(reactPage);
