...
/**
 * CSS API Tests for pfv6-truncate
 *
  ...
  cssApiTests.forEach(({ name, defaultValue, resolvedValue, type, testValue, demo }) => {
    test(`CSS API: ${name}`, async ({ page, browser }) => {
      ...
      try {
        ...
        await reactPage.goto(`/elements/pfv6-truncate/react/test/${demo}`);
        await applyCssOverride(reactPage, '.pf-v6-c-truncate', name, testValue);
        await waitForFullLoad(reactPage);

        // Load Lit demo with CSS override
        await page.goto(`/elements/pfv6-truncate/test/${demo}`);
        await applyCssOverride(page, 'pfv6-truncate', name, testValue);
