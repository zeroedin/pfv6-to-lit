...
async function showPopover(page: Page): Promise<void> {
  ...
  await page.waitForSelector('#pf-popover', { state: 'visible', timeout: 5000 });
...
const litDemos = discoverDemos('popover');
  ...
  litDemos.forEach(demoName => {
    test(`Parity: ${demoName} (Lit vs React)`, async ({ page, browser }) => {
      ...
      try {
        ...
        await reactPage.goto(`/elements/pfv6-popover/react/test/${demoName}`);
        await waitForFullLoad(reactPage);

        await page.goto(`/elements/pfv6-popover/test/${demoName}`);
        await waitForFullLoad(page);

        // Show popover on both pages (popovers are hidden by default)
        await showPopover(reactPage);
