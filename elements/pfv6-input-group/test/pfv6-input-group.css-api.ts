...
/**
 * CSS API Tests for pfv6-input-group
 *
  ...
  cssApiTests.forEach(({
    ...
    test(`CSS API: ${name}`, async ({ page, browser }) => {
      ...
      try {
        ...
        await reactPage.goto(`/elements/pfv6-input-group/react/test/${demo}`);
        await applyCssOverride(reactPage, '.pf-v6-c-input-group', name, testValue);
        await waitForFullLoad(reactPage);

        // Load Lit demo with CSS override
        await page.goto(`/elements/pfv6-input-group/test/${demo}`);
        await applyCssOverride(page, 'pfv6-input-group', name, testValue);
